...
public final class HamsterDietUtil {
     ...
     */
    public static int tryAggressionToggle(HamsterEntity hamster, Player player, ItemStack stack) {
    ...
    private static int trySetAggressionState(HamsterEntity hamster, Player player, ItemStack stack, HamsterEntity.AggressionState targetState) {
        ...
        if (!hamster.level().isClientSide()) {
            ...
            // Audio Feedback
            SoundEvent sound = targetState == HamsterEntity.AggressionState.MENACE ? SoundEvents.ENDER_DRAGON_GROWL : getRandomSoundFrom(ModSounds.HAMSTER_AFFECTION_SOUNDS, hamster.getRandom());
            ...
            hamster.level().playSound(null, hamster.blockPosition(), sound, SoundSource.NEUTRAL, volume, pitch);

            // Visual Feedback
            ParticleOptions particle = targetState == HamsterEntity.AggressionState.MENACE ? ParticleTypes.ANGRY_VILLAGER : ParticleTypes.HAPPY_VILLAGER;
            ...
            player.sendOverlayMessage(Component.translatable(msgKey).withStyle(ChatFormatting.WHITE));
            ...
            if (targetState == HamsterEntity.AggressionState.MENACE) {
                ...
                float damageAmount = (float) hamster.getAttributeValue(Attributes.ATTACK_DAMAGE);
