...
public final class HamsterInteractionUtil {
    ...
    public static InteractionResult handleAccessoryInteraction(HamsterEntity hamster, Player player, ItemStack stack, InteractionHand hand) {
        if (hamster.canPlaceItem(HamsterInventoryUtil.ACCESSORY_SLOT_INDEX, stack) && !player.isShiftKeyDown()) {
            if (!hamster.level().isClientSide()) {
                ...
                if (stack.is(ItemTags.FLOWERS) && currentAccessory.is(ItemTags.FLOWERS) && ItemStack.isSameItem(stack, currentAccessory)) {
                    ...
                    hamster.level().playSound(null, hamster.blockPosition(), SoundEvents.PINK_PETALS_PLACE, SoundSource.PLAYERS, 0.7f, 1.0f + hamster.getRandom().nextFloat() * 0.2f);
                    ParticleEffectsUtil.spawnParticles(hamster.level(), new Vec3(hamster.getX(), hamster.getY() + hamster.getBbHeight() * 0.75, hamster.getZ()), ParticleTypes.FALLING_SPORE_BLOSSOM, 7, new Vec3(hamster.getBbWidth() / 2.0, hamster.getBbHeight() / 2.0, hamster.getBbWidth() / 2.0), 0.0);
                } else {
                    ...
                    if (!toReturn.isEmpty()) {
                        hamster.spawnAtLocation((ServerLevel) hamster.level(), toReturn);
                    }

                    hamster.level().playSound(null, hamster.blockPosition(), SoundEvents.ARMOR_EQUIP_GENERIC.value(), SoundSource.PLAYERS, 1.0f, 1.0f);
                    ParticleEffectsUtil.spawnParticles(hamster.level(), new Vec3(hamster.getX(), hamster.getY() + hamster.getBbHeight() * 0.75, hamster.getZ()), new ItemParticleOption(ParticleTypes.ITEM, toEquip.getItem()), 7, new Vec3(hamster.getBbWidth() / 2.0, hamster.getBbHeight() / 2.0, hamster.getBbWidth() / 2.0), 0.0);

                    if (toEquip.is(ItemTags.FLOWERS) && player instanceof ServerPlayer serverPlayer) {
    ...
    public static InteractionResult handleShearing(HamsterEntity hamster, Player player, ItemStack stack, InteractionHand hand) {
        if (stack.is(Items.SHEARS) && !player.isShiftKeyDown()) {
            ...
            if (!armorStack.isEmpty() && armorStack.getItem() instanceof HamsterArmorItem) {
                actionTaken = true;
                if (!world.isClientSide()) {
                    hamster.spawnAtLocation((ServerLevel) world, armorStack);
            ...
            if (!actionTaken && !accessoryStack.isEmpty()) {
                ...
                if (!world.isClientSide()) {
                    ...
                    hamster.spawnAtLocation((ServerLevel) world, accessoryStack);
    ...
    // --- Aggression Toggle ---
    public static InteractionResult handleAggressionToggle(HamsterEntity hamster, Player player, ItemStack stack, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown()) {
            if (!hamster.level().isClientSide()) {
                if (hamster.isCheekPouchUnlocked() || !AdorableHamsterPets.MAIN_CONFIG.requireFoodMixToUnlockCheeks) {
                    ((ServerPlayer) player).openMenu(new HamsterScreenHandlerFactory(hamster));
                } else {
