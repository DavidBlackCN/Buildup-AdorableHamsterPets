...
public final class HamsterMovementUtil {
    ...
    public static void tryTeleportTo(HamsterEntity hamster, Entity target) {
        ...
        if (Configs.AHP_MAIN.enableTeleportRescue && hamster.distanceToSqr(target) > 1024.0) {
            ...
            if (ownerPlayer instanceof PlayerEntityAccessor accessor) {
                ...
                hamster.saveWithoutId(output); // Save full state
                CompoundTag nbt = output.buildResult();

                // Save target (parent or player)
                nbt.store("AHPTransitTargetUuid", net.minecraft.core.UUIDUtil.CODEC, target.getUUID());
        ...
        safePosOpt.ifPresent(safePos -> {
            hamster.snapTo(safePos.getX() + 0.5, safePos.getY(), safePos.getZ() + 0.5, hamster.getYRot(), hamster.getXRot());
