...
public class AnnouncementScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        matrices.translate(this.width / 2.0F, this.guiTop + Math.round(9 * this.uiScale));
        ...
        if (event.button() == 0) {
            ...
            if (clickedText != null) {
                ...
                if (style.getClickEvent() != null) {
                    Screen.defaultHandleClickEvent(style.getClickEvent(), this.minecraft, this);
                    return true;
                }
            }
        }
    ...
    private Style styleAtWidth(FormattedCharSequence text, int targetX) {
        ...
        text.accept((index, style, codePoint) -> {
            float glyphWidth = this.minecraft.font.width(new String(Character.toChars(codePoint)));
            if (targetX >= currentX[0] && targetX < currentX[0] + glyphWidth) {
                result[0] = style;
                return false;
            }
            currentX[0] += glyphWidth;
            return true;
        });
        return result[0];
    }
