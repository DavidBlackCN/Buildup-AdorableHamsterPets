...
public class HamsterProjectileEntity extends ThrowableProjectile {
    ...
    public static final EntityDataAccessor<CompoundTag> HAMSTER_DATA = SynchedEntityData.defineId(HamsterProjectileEntity.class, ModEntityDataSerializers.COMPOUND_TAG);
    ...
    public HamsterProjectileEntity(Level world, LivingEntity owner) {
        super(ModEntities.HAMSTER_PROJECTILE.get(), owner.getX(), owner.getEyeY() - 0.1, owner.getZ(), world);
        this.setOwner(owner);
    }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        output.store("HamsterData", CompoundTag.CODEC, this.getHamsterData());
        output.putBoolean("HasPlayedIncomingSound", this.hasPlayedIncomingSound);
    }

    @Override
    protected void readAdditionalSaveData(ValueInput input) {
        super.readAdditionalSaveData(input);
        input.read("HamsterData", CompoundTag.CODEC).ifPresent(this::setHamsterData);
        this.hasPlayedIncomingSound = input.getBooleanOr("HasPlayedIncomingSound", false);
    }
        ...
        if (!hamsterNbt.isEmpty()) {
            ...
            if (hamster != null) {
                ...
                if (hitEntity instanceof ArmorStand) {
                    ...
                    boolean damaged = livingHit.hurtServer((ServerLevel) this.level(), damageSource, damageAmount); // Hamster is damage source
            ...
            if (hamster != null) {
                ...
                if (TreeHeistUtil.isValidHeistStartBlock(hitState)) {
                    ...
                    if (HamsterTreeSearcherEntity.isBlockOccupied(this.level(), scanResult.treeId())) {
                        ...
                        HamsterTreeSearcherEntity searcher = ModEntities.HAMSTER_TREE_SEARCHER.get().create(this.level(), EntitySpawnReason.MOB_SUMMONED);
                        if (searcher != null) {
                            TagValueOutput output = TagValueOutput.createWithContext(ProblemReporter.DISCARDING, hamster.registryAccess());
                            hamster.saveWithoutId(output); // Capture full entity state (Owner, Attributes, etc.)
