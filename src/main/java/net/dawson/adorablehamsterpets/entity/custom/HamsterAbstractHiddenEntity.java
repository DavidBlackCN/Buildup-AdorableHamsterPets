...
public abstract class HamsterAbstractHiddenEntity extends Entity {
    ...
    public boolean isOwnedBy(Player player) {
        return this.hamsterNbt != null
                && this.hamsterNbt.read("Owner", UUIDUtil.CODEC)
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        output.store("HamsterNBT", CompoundTag.CODEC, this.hamsterNbt);
        if (this.anchorPos != null) {
            output.putLong("AnchorPos", this.anchorPos.asLong());
        }
        if (this.forcedExitPos != null) {
            output.putLong("ForcedExitPos", this.forcedExitPos.asLong());
        }
        if (this.forcedExitYaw != null) {
            output.putFloat("ForcedExitYaw", this.forcedExitYaw);
        }
    }

    @Override
    protected void readAdditionalSaveData(ValueInput input) {
        this.hamsterNbt = input.read("HamsterNBT", CompoundTag.CODEC).orElseGet(CompoundTag::new);

        if (input.contains("AnchorPos")) {
            this.anchorPos = BlockPos.of(input.getLongOr("AnchorPos", 0L));
        } else if (input.contains("TreeAnchor")) {
            // Backwards compatibility for pre v3.6.1 tree heists
            this.anchorPos = BlockPos.of(input.getLongOr("TreeAnchor", 0L));
        }

        if (input.contains("ForcedExitPos")) {
            this.forcedExitPos = BlockPos.of(input.getLongOr("ForcedExitPos", 0L));
        }
        if (input.contains("ForcedExitYaw")) {
            this.forcedExitYaw = input.getFloatOr("ForcedExitYaw", 0.0f);
        }
    }
    ...
    protected HamsterEntity popOut(boolean success) {
        ...
        HamsterEntity newHamster = ModEntities.HAMSTER.get().create(serverWorld, EntitySpawnReason.MOB_SUMMONED);
        if (newHamster != null) {
